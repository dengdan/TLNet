...
#         pdb.set_trace()
    ...
    def build(self):
        ...
        with tf.variable_scope('avod_projection'):
            ...
            with tf.variable_scope('img_r'):
                image_r_shape = tf.cast(tf.shape(
        ...
        with tf.variable_scope('avod_roi_pooling'):
            ...
            img_boxes_norm_batches = tf.expand_dims(
            ...
            img_rois = tf.image.resize_bilinear(img_rois, self._proposal_roi_crop_size) * cos_simi
            img_r_rois = tf.image.resize_bilinear(img_r_rois, self._proposal_roi_crop_size) * cos_simi
        ...
        with tf.variable_scope('img_r_avod_rois'):
            ...
            img_r_input_rois = tf.image.crop_and_resize(
                self._rpn_model._img_r_preprocessed,
                mb_img_r_anchors_norm,
                mb_img_r_box_indices,
                (32, 32))
    ...
    def loss(self, prediction_dict):
        ...
        losses_output, avod_acc_all, avod_acc_pos = avod_loss_builder.build(self, prediction_dict)
